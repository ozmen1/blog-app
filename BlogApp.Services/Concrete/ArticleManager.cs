...
namespace BlogApp.Services.Concrete
{
    public class ArticleManager : IArticleService
    {
        ...
        {
            var article = _mapper.Map<Article>(articleAddDto); //automapping
        ...
        {
            var article = _mapper.Map<Article>(articleUpdateDto); //articleupdatedto'yu article generic tipinde article'a aktar
            article.ModifiedByName = modifiedByName;
            var addedArticle = await _unitOfWork.Articles.UpdateAsync(article);
            await _unitOfWork.SaveAsync();
            return new DataResult<ArticleDto>(ResultStatus.Success, $"{articleUpdateDto.Title} başlıklı kategori başarıyla güncellenmiştir.", new ArticleDto
            {
                Article = addedArticle,
                ResultStatus = ResultStatus.Success,
                Message = $"{articleUpdateDto.Title} başlıklı kategori başarıyla güncellenmiştir."
            });
